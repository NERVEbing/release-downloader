...
func main() {
	...
	flagRepository := flag.String("repository", "", "github repository, {owner}/{repo}")
	flagTag := flag.String("tag", "", "optional, default: '', download assets from a specific tag (eg: regexp '.*.18.*')")
	flagFilename := flag.String("filename", "", "optional, default: '', download assets from a specific filename, excluding tarball or zipball (eg: regexp '.*linux-arm64.*.gz')")
	flagLatest := flag.Bool("latest", false, "optional, default: false, download target as latest release")
	flagPrerelease := flag.Bool("prerelease", false, "optional, default: false, download target as prerelease")
	flagToken := flag.String("token", "", "optional, default: '', github personal access token")
	flagPath := flag.String("path", "./tmp", "optional, default: './tmp', download path")
	flagInterval := flag.Duration("interval", DefaultInterval, "optional, default: 1h, download task interval")
	flogNow := flag.Bool("now", false, "optional, default: false, immediately run")
	flag.Parse()
